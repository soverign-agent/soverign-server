-- Canonical baseline schema for Sovereign AI Compliance Platform
-- Creates a fully migrated PostgreSQL 16 + pgvector database from scratch.
-- All statements are idempotent (IF NOT EXISTS / IF EXISTS) so the file can
-- safely be re-run on a database that already has most objects.
...
-- Triggers on tables that need auto-updated_at
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_audit_jobs_set_updated_at') THEN
        CREATE TRIGGER trigger_audit_jobs_set_updated_at
            BEFORE UPDATE ON audit_jobs
            FOR EACH ROW EXECUTE FUNCTION set_updated_at();
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_approval_requests_set_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_generated_documents_set_updated_at') THEN
        CREATE TRIGGER trigger_generated_documents_set_updated_at
